...
import styles from './MyPackagesScreen.module.css';
...
const PackageCard = ({ item }) => (
  <Link to={`/app/parcel/${item.id}`} className={styles.packageCard}>
    <div className={styles.packageId}>{item.id}</div>
...
export default function MyPackagesScreen() {
  ...
  const filteredPackages = useMemo(() => {
    ...
    if (activeTab !== 'All') {
      packages = packages.filter(p => p.status.toLowerCase().includes(activeTab.toLowerCase().replace(' ', '')));
    }
    if (searchQuery) {
